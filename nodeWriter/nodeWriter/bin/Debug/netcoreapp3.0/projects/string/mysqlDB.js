...
/**
 * 
 * @param {object} deleteOptions the tablename and where clause
 * @param {string} deleteOptions.tableName the table name in the Database
 * @param {string} deleteOptions.where the where clause
 * @param {function} resolve the callback when things go swell
 * @param {function} reject the callback when things go belly up
 */
DB.deleteRowsInTable = function(deleteOptions, resolve, reject) {
    if (DB.mysql_isConnected == false) {
        DB.connect(DB.host, DB.user, DB.password, DB.database);
    }

    //DELETE FROM table_name WHERE condition;
    var tableName = deleteOptions.tableName;
    var where = deleteOptions.where;
